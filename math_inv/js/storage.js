const DB_NAME = "MathematicsInvestigationBuilderDB";
const DB_VERSION = 1;
const STORE_NAME = "investigations";
const RECORD_KEY = "current";
const LEGACY_LOCAL_STORAGE_KEY = "mathematicsInvestigationBuilder.v1";
...
function parseSavedPayload(raw) {
  ...
  try {
    ...
    console.error("Could not parse saved mathematics investigation.", error);
    return null;
  }
}
...
export function exportAsJson(state) {
  const payload = {
    ...
    app: "Mathematics Investigation Builder",
    state,
  };
  ...
  link.href = url;
  link.download = `mathematics-investigation-${timestamp}.json`;
