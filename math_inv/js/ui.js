import { SECTION_ORDER } from "./validation.js";
...
export function createUI() {
  return {
    ...
    investigationCycles: document.getElementById("investigation-cycles"),
    ...
    graphCards: document.getElementById("graph-cards"),
    strengthRows: document.getElementById("strength-rows"),
    limitationRows: document.getElementById("limitation-rows"),
    improvementRows: document.getElementById("improvement-rows"),
    introductionPreview: document.getElementById("introduction-paragraph-preview"),
...
export function renderInvestigationCycles(container, cycles) {
  container.innerHTML = cycles
    .map(
      (cycle, index) => `
        <article class="dynamic-row investigation-cycle" data-cycle-index="${index}">
          <div class="cycle-header">
            ...
            ${
              index > 0
                ? `<button class="secondary" type="button" data-action="remove-investigation-cycle" data-index="${index}">Remove cycle</button>`
                : ""
            }
          </div>
          <label>
...
export function initialiseInvestigationSortable(container, onReorder) {
...
export function renderGraphCards(container, graphs, data) {
  const displayColumns = getDisplayColumns(data);
  const cards = Array.isArray(graphs?.cards) ? graphs.cards : [];
      ...
      return `
        <div class="analysis-graph-card">
          <div class="analysis-graph-header">
            ...
            ${
              index > 0
                ? `<button class="secondary" type="button" data-action="remove-graph-card" data-index="${index}">Delete graph</button>`
                : ""
            }
          </div>
          <div class="grid two-col analysis-graph-controls">
            <label>
              Graph type
              <select data-array="graphs.cards" data-index="${index}" data-field="graphType">
                ${graphTypeOptions}
              </select>
            </label>
            <label>
              Trendline
              <select data-array="graphs.cards" data-index="${index}" data-field="trendlineType"${supportsTrendline ? "" : " disabled"}>
            ...
            <label>
              X-axis column
              <select data-array="graphs.cards" data-index="${index}" data-field="xColumn" data-value-type="number">${xOptions}</select>
            </label>
            <label>
              Y-axis column(s)
              <select
            ...
            <label class="checkbox-label">
              ...
              />
              Force compatible axes to start at zero
            </label>
          </div>
          <p class="support-text">Hold Ctrl (Windows) or Cmd (Mac) to select multiple Y-axis columns. ${trendlineHint}${bubbleHint}</p>
          <div class="chart-wrap">
            <canvas data-graph-canvas="${index}" aria-label="Mathematics investigation graph ${index + 1}"></canvas>
          </div>
...
function renderFactorRows(container, rows, config) {
    ...
    .map(
      (row, index) => `
        <div class="dynamic-row factor-row">
          ...
          ${
            index > 0
              ? `<button class="secondary" type="button" data-action="${escapeAttr(config.removeAction)}" data-index="${index}">Remove</button>`
              : ""
          }
        </div>
      `,
...
export function renderStrengthRows(container, rows) {
  renderFactorRows(container, rows, {
    arrayPath: "analysis.strengths",
    primaryField: "factor",
...
export function renderLimitationRows(container, rows) {
  renderFactorRows(container, rows, {
    arrayPath: "analysis.limitations",
    primaryField: "factor",
    secondaryField: "impact",
...
export function renderImprovementRows(container, rows) {
  renderFactorRows(container, rows, {
    arrayPath: "analysis.improvements",
    primaryField: "improvement",
...
export function renderExportChecklist(container, results) {
  const labels = {
    ...
    investigation: "Mathematical investigation",
    data: "Data tables",
    graphs: "Graphs",
    analysis: "Analysis",
...
export function setIntroductionPreview(element, text) {
  element.innerHTML = text
    ? renderMathNotationHtml(text)
    : escapeHtml(
        "Your introduction preview will appear here when you complete the hook, problem, field of mathematics, and connection.",
      );
}
...
export function setConclusionPreview(element, text) {
  element.innerHTML = text
    ? renderMathNotationHtml(text)
    : escapeHtml("Your conclusion preview will appear here when you complete the concluding fields.");
}
